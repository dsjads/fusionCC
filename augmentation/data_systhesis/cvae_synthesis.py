...
from augmentation.data_systhesis.CVAE_model import *
# from augmentation.data_systhesis.CVAE_model_mlp import *
...
class CVAESynthesisData(ProcessedData):
    ...
    def process(self):
        ...
        if len(fail_feature) == 1:
            self._copy_failed(pass_feature,fail_feature,diff_num)
            return

        self._generate_failed(pass_feature,fail_feature,diff_num)




    def _copy_failed(self,pass_feature,fail_feature,diff_num):
        features_np = np.array(self.feature_df)
        generated_failed = np.zeros((diff_num,len(self.feature_df.columns)),dtype=float)
        for i in range(diff_num):
            generated_failed[i] = fail_feature
        compose_feature = np.vstack((features_np,generated_failed))
        self.generated_failed = pd.DataFrame(generated_failed, columns=self.feature_df.columns, dtype=float)
    ...
    def _generate_failed(self,pass_feature,fail_feature,diff_num):
        min_batch = 40
        ...
        parser.add_argument("--latent_size", type=int, default=50)  # latent size need to be changed in different program
        ...
        self.generated_failed = pd.DataFrame(x, columns=self.feature_df.columns, dtype=float)
