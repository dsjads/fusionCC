...
class CalculateSuspiciousness():
    def __init__(self, data_obj, method, save_rank_path, experiment):
        ...
        self.method = method
        self.save_rank_path = save_rank_path
    ...
    def _save_rank(self):
        save_rank_filename = os.path.join(self.save_rank_path, f"{self.state}_MFR.txt")
        write_rank_to_txt(self.rank_MFR_dict, save_rank_filename, self.data_obj.program, self.data_obj.bug_id)

        save_rank_filename = os.path.join(self.save_rank_path, f"{self.state}_MAR.txt")
