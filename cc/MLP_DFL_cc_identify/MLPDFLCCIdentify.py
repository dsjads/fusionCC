...
class CBNNCCIdentify(BaseCCPipeline):
    ...
    def _find_cc_index(self):
        ...
        for i in range(0, program_len, K_size):
            if (i + K_size > program_len):
                for j in range(i, program_len):
                    config = {'-d': 'd4j', '-p': program, '-i': str(info_list[j]), '-m': program_method,
                              '-e': 'origin'}
                    self.train_test_config_list[math.floor(i / K_size)].append(config)
            else:
                for j in range(i, i + K_size):
                    config = {'-d': 'd4j', '-p': program, '-i': str(info_list[j]), '-m': program_method,
                              '-e': 'origin'}
                    self.train_test_config_list[math.floor(i / K_size)].append(config)
        ...
        for item in self.train_test_config_list:
            ...
            save_path = os.path.join(project_dir, "results", "2022-09-20-CBNN", "time.txt")
