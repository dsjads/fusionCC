...
class CCPrecisionCleanPipeline(ReadData):
    ...
    def cc_survey_percent(self):
        self._clean_cc_percent()
        self._calRes("2022525survey_clean_cc_precision"+str(self.precision_percent))

    def set_precision_percent(self, precision_percent):
        self.precision_percent = precision_percent

    def _clean_cc_percent(self):
...
def main():
    ...
    for program, program_version_num in zip(program_list, program_version_num_list):
        for i in range(1, program_version_num + 1):
            ...
            # ccspl.cc_survey()
            ccsp = CCPrecisionCleanPipeline(project_dir, configs)
            # ccsp.cc_num_survey()
            # for p in range(10, 101, 10):
            #    ccsp.set_precision_percent(p)
            #    ccsp.cc_survey_percent()
            ccsp.set_precision_percent(0)
            ccsp.cc_survey_percent()

if __name__ == "__main__":
    main()
    task_complete("precision 0 end")
