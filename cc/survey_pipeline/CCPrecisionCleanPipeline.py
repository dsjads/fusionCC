...
class CCPrecisionPipeline(BaseCCPipeline):
    ...
    def clean(self, passing_df, failing_df):
        ...
        self.way = str(self.precision_percent)+"-precision-trim"
        self.calRes("trim")

    def relabel(self, passing_df, failing_df):
        passing_df["error"][self.cc_index] = 1
        cc_data_df = pd.concat([passing_df, failing_df])
        self.way = str(self.precision_percent)+"-precision-relabel"
        self.data_obj.reload(cc_data_df)
        self.calRes("relabel")
    ...
    for program in program_list:
        for i in cc_info[program]:
            ...
            if flag:
                ...
                # ccspl.cc_survey()
                ccsp = CCPrecisionPipeline(project_dir, configs, "precision")
                # ccsp.cc_num_survey()
